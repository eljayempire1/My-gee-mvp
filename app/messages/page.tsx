...
function getSavedMessages(id: string): Message[] {
  try {
    const value = JSON.parse(localStorage.getItem(`gee-v2-messages-${id}`) || "[]");
...
function getDemoConnections(): string[] {
  try {
    const value = JSON.parse(localStorage.getItem("gee-demo-connections") || "[]");
...
function fallbackReply(name: string, text: string) {
  ...
  if (/lonely|alone|nobody|no one|isolated/.test(lower)) {
    return `Ahh ${name} ❤️ I'm sorry you're feeling lonely. Come, talk to me for a bit — what's been making you feel this way? 🫂`;
  }
  if (/sad|upset|hurt|cry|bad day|not okay|stressed|overwhelmed/.test(lower)) {
    return `Aww ❤️ I'm sorry you're having a rough time. You don't have to brush it off with me — what happened? 🫂`;
  }
  if (/happy|excited|great|amazing|good news|good day/.test(lower)) {
    return `Yesss 😄❤️ I love hearing that energy from you. Tell me what happened!`;
  }
...
function MessagesContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      else if (localStorage.getItem("gee-demo-session")) setUserId("demo-user");

      const map = new Map<string, Person>();
      people.forEach((p) => map.set(p.id, p));
      getDemoConnections().forEach((id) => {
      ...
      try {
        ...
        // Demo directory remains available if Supabase is unavailable.
      }
  ...
  useEffect(() => {
    ...
    if (!requested) return;
    const match = connections.find((p) => p.display_name.toLowerCase() === requested.toLowerCase());
  ...
  useEffect(() => {
    ...
    if (person.id.startsWith("demo-")) {
      setMessages(getSavedMessages(person.id));
      return;
    }
    let mounted = true;
    (async () => {
  ...
  async function send(event: FormEvent) {
    ...
    if (person.id.startsWith("demo-")) {
      ...
      try {
        const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({ companionName: person.display_name, messages: [{ role: "user", content: body }] }),
        });
        ...
        const updated = [
          ...next,
          { id: `g-${Date.now()}`, connection_id: person.id, sender_id: person.id, body: reply, created_at: new Date().toISOString() },
        ];
        setMessages(updated);
    ...
    if (!userId) {
      setNotice("Please sign in to send private messages.");
      return;
    }
  ...
  if (loading) {
    return <><Nav /><main className="page"><p>Loading your connections…</p></main></>;
  }
      ...
      <main className="page">
        <div className="wrap">
          <header className="top">
            <div><div className="eyebrow">MESSAGES</div><h1>Stay connected. 💜</h1><p>Your connections are here — choose someone to chat or call.</p></div>
            <button className="friendsToggle" onClick={() => setDrawer(true)}>☰ Friends</button>
            <input className="searchTop" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search" />
          </header>
          {notice && <div className="notice">{notice}</div>}

          <div className="layout">
            <aside className="friends">
              <b>CONNECTIONS • {filtered.length}</b>
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search friends" />
              {filtered.map((p) => (
                <button key={p.id} onClick={() => setActiveId(p.id)} className={p.id === person?.id ? "friend selected" : "friend"}>
                  <span className="avatar">{p.avatar_letter}</span><span><strong>{p.display_name}</strong><small>● Online</small></span>
                </button>
              ))}
            </aside>

            {drawer && <div className="overlay" onClick={() => setDrawer(false)}><div className="drawer" onClick={(e) => e.stopPropagation()}><div className="drawerHead"><b>Connections</b><button onClick={() => setDrawer(false)}>✕</button></div>{filtered.map((p) => <button key={p.id} onClick={() => { setActiveId(p.id); setDrawer(false); }} className="friend"><span className="avatar">{p.avatar_letter}</span><span><strong>{p.display_name}</strong><small>● Online</small></span></button>)}</div></div>}

            <section className="chat">
              <header className="chatHead">
                <button className="mobileMenu" onClick={() => setDrawer(true)}>☰</button>
                <span className="avatar">{person?.avatar_letter || "G"}</span>
                <div className="person"><strong>{person?.display_name || "Friend"}</strong><small>● Online • {person?.city || "UK"}</small></div>
                <div className="actions"><a href={`/call?type=voice&name=${encodeURIComponent(person?.display_name || "")}`}>📞</a><a href={`/call?type=video&name=${encodeURIComponent(person?.display_name || "")}`}>🎥</a></div>
              </header>
              <div className="body">
                ...
                {visible.map((m) => <div key={m.id} className={m.sender_id === userId || m.sender_id === "guest" || m.sender_id === "demo-user" ? "row mine" : "row"}><div className="bubble">{m.body}<small>{new Date(m.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</small></div></div>)}
                {replying && <div className="typing">{person?.display_name} is thinking… 💜</div>}
              </div>
              <form className="composer" onSubmit={send}><button type="button" onClick={() => setText((v) => `${v} 💜`)}>😊</button><input value={text} onChange={(e) => setText(e.target.value)} placeholder={`Message ${person?.display_name || "friend"}...`} /><button className="send" disabled={!text.trim() || replying}>➤</button></form>
            </section>
          </div>
        </div>
      </main>
