...
function normalizePeople(rows:Person[]){
  return rows.map(p=>{const name=(p.display_name||"").trim().toLowerCase();if(name==="michael"||name==="micheal")return {...p,display_name:"Elijah OBONOGWU",bio:p.bio||"Entrepreneurial, relaxed and into great food, music and good vibes.",interests:p.interests?.length?p.interests:["Business","Music","Food"]};return p;});
}
...
export default function Discover(){
 ...
 async function connect(person:Person){if(connecting)return;setConnecting(person.id);
   ...
   if(!user&&!demoSession){setNotice("Please sign in first. 💜");setConnecting("");setTimeout(()=>setNotice(""),2500);return}
   ...
   if(!user){setNotice("This member needs a real account connection. 💜");setConnecting("");return}
