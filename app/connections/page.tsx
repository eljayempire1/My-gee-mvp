...
const initial: Person[] = [
  ...
  { id: "demo-sarah", name: "Sarah", gender: "Woman", status: "Connected", note: "Travel • Art • Business" },
  { id: "demo-olivia", name: "Olivia", gender: "Woman", status: "Connected", note: "Books • Coffee • Travel" },
  { id: "demo-sophia", name: "Sophia", gender: "Woman", status: "Request", note: "Fitness • Fashion • Food" },
  { id: "demo-david", name: "David", gender: "Man", status: "Connected", note: "Technology • Business • Fitness" },
  ...
  return <><Nav /><main style={{ minHeight: "100vh", padding: "22px 14px 40px", fontFamily: "Arial, sans-serif", background: "linear-gradient(180deg,#07070a,#110a17)" }}>
    <div style={{ maxWidth: 980, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(270px,1fr))", gap: 14, marginTop: 24 }}>
        {filtered.map((p) => <div key={p.id} style={{ padding: 18, background: "linear-gradient(145deg,#17101e,#101014)", border: "1px solid #332a39", borderRadius: 20, boxShadow: "0 12px 35px #0005" }}>
          ...
          <div style={{ display: "flex", gap: 7 }}>
            ...
            <button type="button" onClick={() => toggleConnection(p.id)} style={{ ...action, cursor: "pointer", background: p.status === "Connected" ? "#24152d" : "linear-gradient(135deg,#7c3aed,#c026d3)" }}>{p.status === "Connected" ? "✓ Connected" : "💜 Connect"}</button>
          </div>
        </div>)}
      </div>
    </div>
